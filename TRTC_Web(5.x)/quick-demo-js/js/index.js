...
document.getElementById("userId").value =
  getQueryString("userId") || Math.floor(Math.random() * 1000000);
document.getElementById("roomId").value =
  getQueryString("roomId") || Math.floor(Math.random() * 1000);
...
if (!window.isIframe) initDevice();
  ...
  document.getElementById("user-sig-doc").querySelector("a").href =
    "https://trtc.io/zh/document/35166?platform=web&product=rtcengine&menulabel=sdk";
}
handleEvent();
...
function initParams() {
  ...
  if (!(sdkAppId && sdkSecretKey && roomId && userId)) {
    if (window.lang_ === "zh-cn") {
      ...
      alert(
        "Please fill in the correct SDKAppId, SDKSecretKey, userId, roomId!"
      );
    }
...
async function startBeautyAR() {
  const APPID = ""; // todo:  输入美颜 license 对应的 appid 
  const LICENSE_KEY = ""; // todo: 输入美颜 license
  const token = ""; // todo: 输入美颜 license 对应的 token
  ...
  // 获取 trtc 的视频流，提供给 ar sdk 处理
  const arInputStream = new MediaStream([trtc.getVideoTrack()]);

  // debug: 渲染 ar 处理前的流
  //   const video = document.createElement("video");
  //   video.setAttribute("id", "webar-input-video");
  //   video.setAttribute("playsinline", "");
  //   video.setAttribute("autoplay", "");
  //   video.setAttribute("style", "position:fixed;top:200px;width:100px;");
  //   video.srcObject = arInputStream;
  //   document.body.appendChild(video);
  //   video.play();

  ar = new AR.ArSdk({ // 访问 window.AR 对象的 ArSdk 类
    input: arInputStream,
    module: {
      beautify: true, // 是否开启美颜
      segmentation: true, // 是否开启人像分割（虚拟背景）
    },
    auth: {
  ...
  ar.on("created", () => {
    ...
    ar.getEffectList({
      ...
      .then((res) => {
        ...
        const makeupList = list.filter(
          (item) => item.label.indexOf("美妆") >= 0
        );
        const stickerList = list.filter(
          (item) => item.label.indexOf("贴纸") >= 0
        );
        console.log("内置美妆列表", makeupList);
        console.log("内置贴纸列表", stickerList);
      })
      .catch((e) => {
        console.log(e);
      });
  ...
  ar.on("ready", async (e) => {
    ...
    // video.setAttribute("playsinline", "");
    // video.setAttribute("autoplay", "");
    // video.setAttribute("style", "position:fixed;top:0;width:100px;");
    ...
    ar.setEffect([
      {
        id: "9C7E317E53997405", //  上述内置美妆或者贴纸的 ID
        intensity: 1,
      }
    ]);
  });

  ar.on("error", async (error) => {
    console.log(error);
  });
}
  ...
  if (trtc) {
    try {
      await trtc.startLocalAudio({
        option: { microphoneId: microphoneSelect.value },
      });
      audio = true;
      isMicOpened = true;
...
async function initDevice() {
  try {
    try {
      ...
      if (window.lang_ === "en") {
        window.alert(
          "If you do not allow the current page to access the microphone and camera permissions, you may fail when publishing a local audio/video."
        );
      } else {
        window.alert(
          "如果不允许当前页面访问麦克风和摄像头权限，您在发布本地音视频流的时候可能会失败。"
        );
      }
      enterBtn.disabled = true;
    }
  ...
  audioDiv.addEventListener("click", async () => {
    if (audio) {
      try {
        ...
        addFailedLog(
          `[${userId}] updateLocalAudio failed. Reason: ${e.message}`
        );
      }
    } else {
      try {
  ...
  videoDiv.addEventListener("click", async () => {
    if (video) {
      try {
        ...
        addFailedLog(
          `[${userId}] updateLocalVideo failed. Reason: ${e.message}`
        );
      }
    } else {
      try {
